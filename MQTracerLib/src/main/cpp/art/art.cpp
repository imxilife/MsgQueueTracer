...
//                           XDL_TRY_FORCE_LOAD);
  ...
  walk_stack = reinterpret_cast<WalkStack_t>(xdl_dsym(handle,
                                                      ...
                                                      nullptr));
  LOGV("ArtHelper","walk_stack %p,",walk_stack);
  if (walk_stack == nullptr) {
    LOGI("ArtHelper","return walk_stack is nullptr");
    return -1;
  }
...
int ArtHelper::init(JNIEnv *env) {
  ...
  auto *javaVMExt = reinterpret_cast<JavaVMExt *>(javaVM); //JavaVM vs JavaVMExt 详见 https://kfft6vo8un.feishu.cn/docx/XsrSdQ7XgojSiMxpPPHcRJLHnAh
  ...
  if (api_level >= ANDROID_API_TIRAMISU) {
    ArtHelper::runtime_instance_ = reinterpret_cast<char *>(runtime) + offsetOfVmExt - offsetof(PartialRuntimeTiramisu, java_vm_);
    thread_list = reinterpret_cast<PartialRuntimeTiramisu *>(ArtHelper::runtime_instance_)->thread_list_;
  } else if (api_level >= ANDROID_API_R) {
    ArtHelper::runtime_instance_ = reinterpret_cast<char *>(runtime) + offsetOfVmExt - offsetof(PartialRuntimeR, java_vm_);
    thread_list = reinterpret_cast<PartialRuntimeR *>(ArtHelper::runtime_instance_)->thread_list_;
  }
  else if (api_level >= ANDROID_API_Q){
    ArtHelper::runtime_instance_ = reinterpret_cast<char *>(runtime) + offsetOfVmExt - offsetof(PartialRuntimeQ, java_vm_);
    ...
    ArtHelper::runtime_instance_ = reinterpret_cast<char *>(runtime) + offsetOfVmExt - offsetof(PartialRuntimeP, java_vm_);
